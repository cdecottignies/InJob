...
describe('Users', () => {
    ...
    it('DELETE /users/admin/:token/:id --> should delete a user', async () => {
        const res = await request(app)
            .delete('/api/users/admin/${testConfig.tokenAdmin}/1')
        expect(res.statusCode).toEqual(204)
    }),

    it('DELETE /users/:token --> should delete all users', async () => {
        const res = await request(app)
